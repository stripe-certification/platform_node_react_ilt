...
export async function listStudios(userId: string) {
    ...
    if (!studios) throw new Error('Studios not found');

    return studios;
}

export async function deleteStudio(studioId: string) {
    const studio = await dbService.loadData('studios', studioId);
    if (!studio) throw new Error('Studio not found');

    await dbService.deleteData('studios', studioId);

    return { success: true, message: `Studio with ID ${studioId} deleted successfully.` };
}
